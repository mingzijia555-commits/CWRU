...
当前正式方案没有加入 Dropout、噪声增强或随机缩放，而是用固定划分、权重衰减、学习率调度和早停控制训练。优点是实验因素更少、三种模型比较更直接；不足是对实验室数据可能拟合较强，面对真实工业噪声的泛化能力尚未验证。
...
- `L_reg`：Smooth L1 损失，只有 IR、OR、B 故障窗口参与；
- Normal 的回归掩码为假，不会强迫回归头把 Normal 学成某个故障直径。

Smooth L1 在误差小时类似平方误差，在误差很大时又不像 MSE 那样对离群值过分敏感，适合直径回归。
